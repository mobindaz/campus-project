...
describe("Department Service Layer", () => {
  ...
  describe("Authorization enforcement", () => {
    it("throws UnauthorizedError when user is missing", async () => {
      await expect(listDepartmentsService(null)).rejects.toThrow(
        UnauthorizedError
      );
      await expect(
        createDepartmentService(null, {
          name: "Test",
          code: "TEST",
          type: "ACADEMIC",
          description: "",
    ...
    it("throws ForbiddenError when non-admin user lacks permission", async () => {
      ...
      await expect(listDepartmentsService(regularUser)).rejects.toThrow(
        ForbiddenError
      );
      await expect(
        createDepartmentService(regularUser, {
          name: "Test",
          code: "TEST",
          type: "ACADEMIC",
          description: "",
    ...
    it("allows College Admin global role access", async () => {
      ...
      vi.mocked(rbacService.getUserPermissions).mockResolvedValue([]);
      vi.mocked(rbacService.getUserDepartmentScopes).mockResolvedValue([]);
      vi.mocked(departmentRepository.listDepartments).mockResolvedValue([
        mockDepartment,
      ]);
  ...
  describe("Department Creation & Validation", () => {
    ...
    it("creates a department when code and name are unique", async () => {
      vi.mocked(departmentRepository.findDepartmentByCode).mockResolvedValue(
        null
      );
      vi.mocked(departmentRepository.findDepartmentByName).mockResolvedValue(
        null
      );
      vi.mocked(departmentRepository.createDepartment).mockResolvedValue(
        mockDepartment
      );
  ...
  describe("Deletion vs Deactivation guard (Correction #8)", () => {
    ...
    it("performs hard delete when reference count is 0", async () => {
      vi.mocked(departmentRepository.findDepartmentById).mockResolvedValue(
        mockDepartment
      );
      vi.mocked(
        departmentRepository.countDepartmentReferences
      ).mockResolvedValue(0);
      vi.mocked(departmentRepository.deleteDepartment).mockResolvedValue(
        mockDepartment
      );

      const result = await deleteDepartmentService(adminUser, "dept_1");
      expect(result.mode).toBe("DELETED");
      expect(departmentRepository.deleteDepartment).toHaveBeenCalledWith(
        "dept_1"
      );
    });
    ...
    it("performs soft deactivation when references exist (>0)", async () => {
      ...
      expect(departmentRepository.deactivateDepartment).toHaveBeenCalledWith(
        "dept_1"
      );
