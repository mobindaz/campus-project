...
import { exportDepartmentsCsvService } from "@/server/services/department.service";
 ...
 * Re-checks `departments.export` permission server-side.
...
export async function GET(request: Request) {
  try {
    ...
    // Get user permissions for column filtering
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="departments-export-${new Date().toISOString().split("T")[0]}.csv"`,
      },
    });
  } catch (error: unknown) {
