...
export function StudentFormModal({ optionsData }: StudentFormModalProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const def = await getFormDefinitionAction("STUDENT_FORM");
        if (!cancelled) {
        ...
        if (!cancelled) {
          setErrorMsg(
            (err as Error).message || "Failed to load Student Form definition."
          );
        }
      } finally {
  ...
  const handleSubmit = async (data: Record<string, unknown>) => {
    ...
    try {
      // Demonstrates runtime form rendering & data capture
      setSubmittedData(data);
    } catch (err: unknown) {
      setErrorMsg((err as Error).message || "Failed to submit student form.");
    } finally {
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  ...
                  <code className="text-indigo-400">STUDENT_FORM</code>{" "}
                  definition
                </p>
              </div>
              <button
            ...
            {submittedData ? (
              <div className="space-y-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-5 text-emerald-300">
                <div className="flex items-center gap-3 text-lg font-bold text-emerald-400">
                  <CheckCircle2 className="h-6 w-6" /> Student Form Submitted
                  Successfully!
                </div>
