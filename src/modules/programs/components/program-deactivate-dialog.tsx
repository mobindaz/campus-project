...
export interface ProgramDeactivateDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  program?: {
...
export function ProgramDeactivateDialog({
  isOpen,
  onClose,
  onSuccess,
  program,
}: ProgramDeactivateDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setErrorMessage(res.error || "Failed to remove program.");
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-300">
            Are you sure you want to remove or deactivate program{" "}
            <strong className="text-white font-semibold">{program.name} ({program.code})</strong>?
          </p>
          ...
          <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-400 space-y-1">
            ...
            <p>
              • If zero student or batch references exist, the program will be permanently deleted.
            </p>
            <p>
              • If active historical references exist, it will be automatically soft-deactivated instead.
            </p>
          </div>
